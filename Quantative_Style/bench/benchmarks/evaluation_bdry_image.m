...
for t = 1:nthresh,
    ...
    for i = 1:numel(S.groundTruth),
        % compute the correspondence
        [match1,match2] = correspondPixels(bmap, double(S.groundTruth{i}.Boundaries), maxDist);
        % accumulate machine matches
        accP = accP | match1;
        % compute recall
        sumR(t) = sumR(t) + sum(S.groundTruth{i}.Boundaries(:));
